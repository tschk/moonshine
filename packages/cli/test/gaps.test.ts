...
describe("templateManualWork", () => {
  test("returns empty array for non-template frameworks", () => {
    const notes = templateManualWork("next-app", [
      { file: "page.tsx", ok: true },
    ]);
  ...
  test("returns baseline notes for svelte", () => {
    ...
    expect(notes[1]).toContain(
      "Re-run `moonshine adopt --force` after editing a template.",
    );
  });
  ...
  test("returns baseline notes for astro including rejects", () => {
    ...
    expect(notes[1]).toContain(
      "Astro templates that use an imported component",
    );
    ...
    const templates: TemplateFile[] = [
      {
        ...
        route: undefined,
        nodes: 10,
        generated: "src/components/Component.ts",
      },
    ];
    const notes = templateManualWork("vue", templates);
    expect(notes).toHaveLength(3);
    expect(notes[0]).toContain("the Composition API");
    expect(notes[2]).toContain(
      "Component.vue: compiled (10 nodes) but maps to no URL",
    );
